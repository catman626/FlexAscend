...
class OPT(nn.Cell):
    ...
    def loadWeight(self, weightFname):
        ...
        # load mha weight
        eye = np.eye(self.config.hiddenSize, dtype=np.float16)
        eye3 = np.concatenate([eye, eye, eye], axis=0) 
        zeros = np.zeros((self.config.hiddenSize,), dtype=np.float16)
